...
#   group_by(year, genre) %>% 
#   summarise(N = n()) %>% 
#   mutate(freq = round(N/sum(N), 2)) %>% View()
...
songs %>% 
  ...
  select(-N) %>% 
  spread(key = decade, value = freq) %>%  View()
...
songs %>% 
  ...
  mutate(decade = floor_date(date, years(10))) %>% 
  group_by(decade) %>% 
  summarise(characters = mean(characters)) %>% 
  # mutate(freq = round(N/sum(N), 2)) %>% 
  # select(-N) %>% 
  spread(key = decade, value = characters) %>%  View()
...
```
*** It seems that songs contain more lyrics now. ???? 


## _Wordcloud per decade
